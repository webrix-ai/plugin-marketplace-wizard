...
import type { ClaudeHookItem, CursorHookItem, PluginHook, DragPayload } from "@/lib/types";
...
function makeEventHook(platform: "claude" | "cursor", event: string): PluginHook {
  return {
    ...
    scope: "global",
    ...(platform === "cursor"
      ? { cursorCategory: CURSOR_TAB_EVENTS.includes(event) ? "tab" as const : "agent" as const }
      : {}),
  };
}
...
function HandlerIcon({ type }: { type: string }) {
  switch (type) {
    case "http": return <Link className="size-3 shrink-0" />;
    case "prompt": return <MessageSquare className="size-3 shrink-0" />;
    case "agent": return <Bot className="size-3 shrink-0" />;
    default: return <Terminal className="size-3 shrink-0" />;
  }
}

function ScopeIcon({ scope }: { scope?: string }) {
  if (scope === "local") return <FolderOpen className="size-2.5 text-muted-foreground" />;
...
function LocalHookItem({
  ...
  const iconCls = color === "orange"
    ? "bg-orange-500/10 text-orange-400"
    : "bg-blue-500/10 text-blue-400";
  const badgeCls = color === "orange"
  ...
  return (
    ...
    >
      <div className={cn("mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-md", iconCls)}>
        <HandlerIcon type={hook.handlerType} />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <p className="truncate text-xs font-medium">{hook.event}</p>
          <Badge variant="outline" className={cn("h-3.5 shrink-0 px-1 text-[9px]", badgeCls)}>
            {hook.handlerType}
          </Badge>
        </div>
        <div className="flex items-center gap-1.5">
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center gap-1.5 px-2.5 py-1.5 text-left cursor-pointer">
        ...
        <Badge variant="outline" className={cn("h-4 px-1.5 text-[9px]", badgeCls)}>
          {count}
        </Badge>
      </CollapsibleTrigger>
      <CollapsibleContent>
...
function LocalTab({ query }: { query: string }) {
  const { claudeHooks, cursorHooks, hooksLoading, claudeHooksEnabled, cursorHooksEnabled, scanHooks } =
    useWizardStore();
  ...
  const filteredClaude = useMemo((): PluginHook[] => {
    return claudeHooks
      .filter((h) => {
        ...
        const summary = handler.type === "command" ? handler.command
          : handler.type === "http" ? handler.url
          : handler.prompt;
        return (
    ...
    <>
      <Section title="Claude Code" count={filteredClaude.length} color="orange">
        {filteredClaude.length === 0 ? (
          <p className="px-5 pb-2 text-[10px] text-muted-foreground">No Claude Code hooks found</p>
        ) : (
      ...
      <Section title="Cursor" count={filteredCursor.length} color="blue">
        {filteredCursor.length === 0 ? (
          <p className="px-5 pb-2 text-[10px] text-muted-foreground">No Cursor hooks found</p>
        ) : (
          <div className="flex flex-col gap-0.5 px-1.5 pb-2">
...
function EventsTab({ query }: { query: string }) {
  ...
  const hasAny =
    filteredClaude.length + filteredCursorAgent.length + filteredCursorTab.length > 0;

  if (!hasAny) {
    return (
      <div className="flex flex-col items-center gap-2 px-4 py-12 text-center">
        <Zap className="size-6 text-muted-foreground" />
        <p className="text-xs text-muted-foreground">No events match &quot;{query}&quot;</p>
      </div>
    );
  }
  ...
  return (
    <>
      {filteredClaude.length > 0 && (
        <Section title="Claude Code Events" count={filteredClaude.length} color="orange">
          <div className="flex flex-col gap-0.5 px-1.5 pb-2">
      ...
      {filteredCursorAgent.length > 0 && (
        <>
          ...
          <Section title="Cursor Agent Events" count={filteredCursorAgent.length} color="blue">
            <div className="flex flex-col gap-0.5 px-1.5 pb-2">
      ...
      {filteredCursorTab.length > 0 && (
        <>
          ...
          <Section title="Cursor Tab Events" count={filteredCursorTab.length} color="blue" defaultOpen={false}>
            <div className="flex flex-col gap-0.5 px-1.5 pb-2">
      ...
      <div className="px-3 pb-3">
        <div className="relative">
          ...
          <Input
            ...
            placeholder={source === "events" ? "Filter events..." : "Filter hooks..."}
