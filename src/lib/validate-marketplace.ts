...
export function validateMarketplaceName(name: string): ValidationIssue | null {
  ...
  if (!isValidKebabCaseId(trimmed)) {
    return {
      path: "name",
      message:
        "Marketplace name must be kebab-case (lowercase letters, numbers, hyphens only)",
    }
  }
  if (RESERVED_MARKETPLACE_NAMES.has(trimmed)) {
    return {
      path: "name",
      message: `"${trimmed}" is reserved for official use`,
    }
  }
...
export function validateOwner(
  owner: MarketplaceSettings["owner"],
): ValidationIssue[] {
...
export function validatePluginEntry(
  ...
  if (entry.author && !entry.author.name?.trim()) {
    issues.push({
      path: `${pfx}.author.name`,
      message: "Author name is required when author is set",
    })
  }
  if (
    entry.author?.email?.trim() &&
    !EMAIL_LOOSE.test(entry.author.email.trim())
  ) {
    issues.push({
      path: `${pfx}.author.email`,
...
export function validateSkill(skill: Skill, pfx = "skill"): ValidationIssue[] {
  ...
  if (!desc) {
    issues.push({
      ...
      message:
        "Description is required so Claude knows when to apply this skill",
    })
...
function skillDirName(skill: Skill): string | null {
  if (skill.sourceFilePath) {
    const parent = skill.sourceFilePath
      .replace(/\\/g, "/")
      .split("/")
      .slice(-2, -1)[0]
...
export function validateAgent(
  ...
  if (!desc) {
    issues.push({
      path: `${pfx}.description`,
      message:
        "Description is required so Claude knows when to delegate to this agent",
    })
