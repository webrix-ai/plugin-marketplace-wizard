      ...
      for (const handler of handlers) {
        ...
        if (handlerType === "http") {
          ...
          parsedHandler = {
            ...
            model:
              typeof h["model"] === "string" ? h["model"] : undefined,
            timeout:
              typeof h["timeout"] === "number" ? h["timeout"] : undefined,
            statusMessage:
              typeof h["statusMessage"] === "string"
          ...
          parsedHandler = {
            ...
            statusMessage:
              typeof h["statusMessage"] === "string"
                ...
                : undefined,
            once:
              typeof h["once"] === "boolean" ? h["once"] : undefined,
          };
        }
...
function extractCursorHooks(
  ...
  for (const [event, hookConfig] of Object.entries(hooksSection)) {
    ...
    items.push({
      ...
      prompt:
        typeof h["prompt"] === "string" ? h["prompt"] : undefined,
      model:
        typeof h["model"] === "string" ? h["model"] : undefined,
      matcher:
        typeof h["matcher"] === "string" ? h["matcher"] : undefined,
      timeout:
        typeof h["timeout"] === "number" ? h["timeout"] : undefined,
      loop_limit:
...
function walkProjectDirs(
  ...
  function walkDir(dirPath: string, depth: number): void {
    ...
    if (!seenClaude.has(localClaudeSettings) && fs.existsSync(localClaudeSettings)) {
