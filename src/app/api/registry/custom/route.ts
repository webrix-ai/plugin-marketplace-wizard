...
export async function GET(request: Request) {
  try {
    ...
    const allServers: { server: RegistryMcpServer; _meta: Record<string, unknown> }[] = [];
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      if (!res.ok) {
        if (page === 0) {
          return NextResponse.json(
            { error: `Registry returned ${res.status}: ${res.statusText}`, servers: [], total: 0 },
            { status: 502 }
          );
        }
        break;
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Custom registry fetch failed",
        servers: [],
        total: 0,
      },
      { status: 500 }
