    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-muted-foreground">{mcp.sourceApplication}</span>
          {mcp.scope === "global" ? (
...
export function SkillItem({
  ...
  const { frontmatter } = useMemo(() => parseSkillFrontmatter(skill.content), [skill.content]);
...
export function RegistryMcpItem({
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-xs font-medium">{server.title || server.name}</p>
        <p className="truncate text-[10px] text-muted-foreground">
...
export function CollapsibleSection({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger
        className="flex w-full items-center gap-1.5 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground transition hover:text-foreground"
      >
        {open ? (
