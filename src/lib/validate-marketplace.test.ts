...
describe("validatePluginEntry", () => {
  ...
  it("requires plugin name", () => {
    ...
    expect(issues.some((i) => i.message.includes("name is required"))).toBe(
      true,
    )
  })
  ...
  it("requires plugin source", () => {
    ...
    expect(issues.some((i) => i.message.includes("source is required"))).toBe(
      true,
    )
  })
...
describe("validateMcpServer", () => {
  ...
  it("rejects unknown transport type", () => {
    ...
    expect(issues.some((i) => i.message.includes("Unknown transport"))).toBe(
      true,
    )
  })
  ...
  it("requires command for stdio", () => {
    ...
    expect(issues.some((i) => i.message.includes("requires a command"))).toBe(
      true,
    )
  })
  ...
  it("requires config object", () => {
    ...
    expect(issues.some((i) => i.message.includes("config is missing"))).toBe(
      true,
    )
  })
})
...
describe("validateSkill", () => {
  ...
  it("requires description", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("Description is required")),
    ).toBe(true)
  })
...
describe("validatePluginData", () => {
  ...
  it("returns warnings for empty plugin (no mcps/skills)", () => {
    ...
    expect(
      issues.some(
        (i) => i.severity === "warning" && i.message.includes("no MCP"),
      ),
    ).toBe(true)
  })
  ...
  it("warns when version is missing", () => {
    ...
    expect(
      issues.some(
        (i) => i.severity === "warning" && i.message.includes("version"),
      ),
    ).toBe(true)
  })
  ...
  it("skips loading skills", () => {
    ...
    expect(issues.some((i) => i.path.includes("skill") || i.path === "")).toBe(
      false,
    )
  })
})
