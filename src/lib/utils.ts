  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      if (typeof parsed.description === "string")
        fm.description = parsed.description.trim()
    }
  } catch {
...
export function parseAgentFrontmatter(content: string): {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      if (typeof parsed.disallowedTools === "string")
        fm.disallowedTools = parsed.disallowedTools.trim()
      if (typeof parsed.model === "string") fm.model = parsed.model.trim()
      if (typeof parsed.permissionMode === "string")
        fm.permissionMode = parsed.permissionMode.trim()
      ...
      if (typeof parsed.background === "boolean")
        fm.background = parsed.background
      if (typeof parsed.effort === "string") fm.effort = parsed.effort.trim()
      if (typeof parsed.isolation === "string")
        fm.isolation = parsed.isolation.trim()
    }
  } catch {
    // fallback: ignore parse errors
  }
