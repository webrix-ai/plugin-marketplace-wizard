...
if (existsSync(projectDir)) {
  console.log()
  console.log(
    `${RED}  ✗ Directory ${CYAN}${projectName}${RED} already exists${RESET}`,
  )
  console.log()
  process.exit(1)
}
      ...
      manifest.plugins = [
        {
          name: pluginSlug,
          source:
            sub === ".claude-plugin" ? `./plugins/${pluginSlug}` : pluginSlug,
          description: "An example plugin to get you started",
          version: "1.0.0",
        },
      ]
...
console.log(
  `${GREEN}  ✓${RESET} Created example plugin ${DIM}plugins/${pluginSlug}${RESET}`,
)
...
try {
  ...
  execSync('git commit -m "Initial marketplace setup"', {
    cwd: projectDir,
    stdio: "ignore",
  })
...
console.log()
console.log(
  `  ${DIM}Run ${TEXT}npm test${DIM} to validate your marketplace${RESET}`,
)
console.log()
