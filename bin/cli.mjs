...
function showBanner() {
  ...
  console.log(`    ${TEXT}--help${RESET}, ${TEXT}-h${RESET}     Show this help message`);
  console.log(`    ${TEXT}--version${RESET}, ${TEXT}-v${RESET}  Show version number`);
  console.log();
...
function runInit(targetDir) {
  ...
  if (existsSync(cursorManifest) || existsSync(claudeManifest)) {
    console.log(`${YELLOW}  ⚠ Marketplace already initialized in this directory${RESET}`);
  ...
  try {
    ownerName = execSync("git config user.name", { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] }).trim() || dirName;
  } catch {}

  let ownerEmail;
  try {
    ownerEmail = execSync("git config user.email", { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] }).trim() || undefined;
  } catch {}

  const slugName = dirName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  ...
  console.log(`${GREEN}  ✓${RESET} Created ${DIM}.cursor-plugin/marketplace.json${RESET}`);
  console.log(`${GREEN}  ✓${RESET} Created ${DIM}.claude-plugin/marketplace.json${RESET}`);
  console.log(`${GREEN}  ✓${RESET} Created ${DIM}plugins/${RESET} directory`);
  console.log();
  console.log(`${BOLD}  Next steps:${RESET}`);
    console.log(`    ${DIM}$${RESET} ${TEXT}pmw start${RESET}  ${DIM}Open the visual editor${RESET}`);
  console.log();
}
  ...
  return [
    createValidator("marketplace-structure", (dir) => {
      ...
      if (!hasCursor && !hasClaude) {
        issues.push({ severity: "error", path: dir, message: "No marketplace manifest found. Run `pmw init` to create one." });
        return issues;
      }

      if (!hasCursor) issues.push({ severity: "warning", path: ".cursor-plugin/marketplace.json", message: "Missing Cursor marketplace manifest" });
      if (!hasClaude) issues.push({ severity: "warning", path: ".claude-plugin/marketplace.json", message: "Missing Claude marketplace manifest" });
      if (!existsSync(join(dir, "plugins"))) issues.push({ severity: "warning", path: "plugins/", message: "No plugins directory found" });
    ...
    createValidator("marketplace-manifest", (dir) => {
      ...
      for (const sub of [".cursor-plugin", ".claude-plugin"]) {
        ...
        try {
          ...
          issues.push({ severity: "error", path: `${sub}/marketplace.json`, message: `Invalid JSON: ${e.message}` });
          continue;
        }

        if (!manifest.name) {
          issues.push({ severity: "error", path: `${sub}/marketplace.json`, message: "Missing required field: name" });
        } else if (!KEBAB_RE.test(manifest.name)) {
          issues.push({ severity: "error", path: `${sub}/marketplace.json`, message: "Marketplace name must be kebab-case" });
        } else if (RESERVED_NAMES.has(manifest.name)) {
          issues.push({ severity: "error", path: `${sub}/marketplace.json`, message: `"${manifest.name}" is reserved for official use` });
        }

        if (!manifest.owner?.name) {
          issues.push({ severity: "error", path: `${sub}/marketplace.json`, message: "Missing required field: owner.name" });
        }

        if (manifest.owner?.email && !EMAIL_RE.test(manifest.owner.email)) {
          issues.push({ severity: "warning", path: `${sub}/marketplace.json`, message: "Invalid owner email format" });
        }

        if (!Array.isArray(manifest.plugins)) {
          issues.push({ severity: "warning", path: `${sub}/marketplace.json`, message: "Missing or invalid plugins array" });
        }
      }
      return issues;
    }),
    ...
    createValidator("plugin-structure", (dir) => {
      ...
      for (const entry of entries) {
        ...
        const hasCursorPlugin = existsSync(join(pluginDir, ".cursor-plugin", "plugin.json"));
        ...
        if (!KEBAB_RE.test(entry.name)) {
          issues.push({ severity: "error", path: `plugins/${entry.name}`, message: "Plugin directory name must be kebab-case" });
        }
        ...
        if (existsSync(mcpPath)) {
          try {
            ...
            if (!mcpJson.mcpServers || typeof mcpJson.mcpServers !== "object") {
              issues.push({ severity: "error", path: `plugins/${entry.name}/.mcp.json`, message: "Missing or invalid mcpServers object" });
            }
          } catch (e) {
            issues.push({ severity: "error", path: `plugins/${entry.name}/.mcp.json`, message: `Invalid JSON: ${e.message}` });
          }
        }
          ...
          try {
            ...
            for (const skillEntry of skillEntries) {
              ...
              if (!existsSync(join(skillsDir, skillEntry.name, "SKILL.md"))) {
                issues.push({ severity: "warning", path: `plugins/${entry.name}/skills/${skillEntry.name}`, message: "Skill directory missing SKILL.md" });
              }
            }
          } catch {}
        }
